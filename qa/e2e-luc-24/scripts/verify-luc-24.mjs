...
async function main() {
  ...
  try {
    ...
    const libraryState = await listLibrary(page);
    console.log("library after first save:", JSON.stringify(libraryState, null, 2));
    const firstSaveHasFurniture =
      libraryState.some(
        (p) =>
          p.name === "Living Room A" &&
          p.itemCount >= 2 &&
          p.hasImage &&
          typeof p.pixelsPerInch === "number",
      );
    if (!firstSaveHasFurniture) {
      record(
        "AC1",
        "User can save the current plan including furniture additions under a name",
        false,
        `Library missing furniture/image/scale: ${JSON.stringify(libraryState)}`,
      );
    }
    ...
    const resavedClean =
      /Living Room A/.test(afterResave) && !/Living Room A \*/.test(afterResave);
    record(
      "AC1b",
    ...
    record(
      ...
      bothPresent && !sameIdOverwrite && afterSecondSave.length >= 2,
      sameIdOverwrite
        ? `FAIL: Save as reused activePlanId and renamed/overwrote. before=${JSON.stringify(beforeSecondSave)} after=${JSON.stringify(afterSecondSave)}`
        : `before=${JSON.stringify(beforeSecondSave)}; after=${JSON.stringify(afterSecondSave)}; open=${openList.replace(/\n/g, " | ")}`,
    );
    await shot(page, "09-open-list-after-save-as");
    await page.getByRole("dialog").getByRole("button", { name: "Cancel" }).click();
    await pause(page, 600);

    // Secondary coexistence path (Reset + Save as)
    ...
    await openNamedPlan(page, "Office Layout B");
    ...
    const restoredLib = await page.evaluate(() => {
      ...
      return {
        ...
        sessionHasImage: Boolean(session?.plan?.imageDataUrl),
      };
    });
    const uiRestored =
    ...
    await openNamedPlan(page, "Living Room A");
    ...
    record(
      ...
      quotaOk,
      alertVisible ? alertText.replace(/\n/g, " | ") : "No alert banner visible",
    );
    await shot(page, "13-quota-error");

    const prdCriteria = results.filter((r) => /^AC[1-5]$/.test(r.id));
    // Deduplicate AC1 if overwritten — keep last AC1 entry for gate
    const byId = new Map();
    for (const r of prdCriteria) byId.set(r.id, r);
    const gated = ["AC1", "AC2", "AC3", "AC4", "AC5"].map((id) => byId.get(id)).filter(Boolean);
    const summary = {
      issue: "LUC-24",
