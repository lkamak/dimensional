...
const PR_COMMIT =
  process.env.GIT_COMMIT || "29e0dc2ec0b3668e4d501e41668713403438fc15";
const HANDLE_FILL = "#fafaf7";
const HANDLE_STROKE = "#3d5a5b";
...
/** Find the rotation handle Circle (cream fill, teal stroke, above furniture). */
  ...
  return page.evaluate(
    ({ fill: handleFill, stroke: handleStroke }) => {
      ...
      for (const c of circles) {
        ...
        if (fill === handleFill && stroke === handleStroke) {
    ...
    { fill: HANDLE_FILL, stroke: HANDLE_STROKE },
  );
}
...
/** Tools/Select/Calibrate/Wall live in TopBarMenu dropdowns on this PR. */
async function chooseTool(page, toolLabel) {
  const trigger = page.getByRole("button", { name: /^Tools/i });
  await trigger.click();
  await pause(page, 250);
  await page.getByRole("menuitem", { name: toolLabel, exact: true }).click();
...
async function calibrate(page) {
  ...
  if (!(await hint.isVisible().catch(() => false))) {
    await chooseTool(page, "Calibrate");
...
async function countRotateHandles(page) {
  ...
  return page.evaluate(
    ({ fill: handleFill, stroke: handleStroke }) => {
      ...
      return stage
        .find("Circle")
        .filter((c) => c.fill() === handleFill && c.stroke() === handleStroke)
        .length;
    },
    { fill: HANDLE_FILL, stroke: HANDLE_STROKE },
  );
    ...
    ({ fill: handleFill, stroke: handleStroke }) => {
      ...
      for (const g of groups) {
        ...
        const hasHandle = circles.some(
          (c) => c.fill() === handleFill && c.stroke() === handleStroke,
        );
...
async function main() {
  ...
  try {
    ...
    // Stem hit-test (29e0dc2): press on connector line must keep selection / start rotate
    let stemKeptSelection = false;
    if (handlePos && center2) {
      const stemX = handlePos.x;
      const stemY = (handlePos.y + (center2.y - 20)) / 2;
      await page.mouse.move(stemX, stemY);
      await page.mouse.down();
      await pause(page, 350);
      stemKeptSelection = await page.locator("#item-rotation").isVisible();
      await page.mouse.up();
      await pause(page, 500);
      await selectItem(page, "couch");
      handlePos = await handleScreenPos(page);
    ...
    record(
      ...
      noMove && stillSelected && bodyMoved && stemKeptSelection,
      `handleNoMove=${noMove}, stillSelected=${stillSelected}, stemKeptSelection=${stemKeptSelection}, label=${selectedLabel}, bodyMoved=${bodyMoved}, dPos=${Math.hypot(sofa.x - beforeBody.x, sofa.y - beforeBody.y).toFixed(1)}`,
    );
    ...
    // Screen radius ≈ worldRadius * stageScale; code uses radius = 7/scale so screen ≈ 7
    const midScreenR =
    ...
    const minZoomNow = await getStageMeta(page);
    const minScreenR =
    ...
    const maxZoomNow = await getStageMeta(page);
    const maxScreenR =
      maxHandle && maxZoomNow ? maxHandle.radius * maxZoomNow.scale : null;
    ...
    const sizeOk =
      ...
      minScreenR != null &&
      maxScreenR != null &&
      Math.abs(midScreenR - HANDLE_SCREEN_RADIUS) < 0.6 &&
      Math.abs(minScreenR - HANDLE_SCREEN_RADIUS) < 0.6 &&
      Math.abs(maxScreenR - HANDLE_SCREEN_RADIUS) < 0.6;
    record(
      "AC8",
      "The handle remains a usable, visually consistent size at minimum and maximum supported zoom.",
      sizeOk,
      `screenR mid/min/max=${midScreenR?.toFixed?.(2)}/${minScreenR?.toFixed?.(2)}/${maxScreenR?.toFixed?.(2)} (target≈${HANDLE_SCREEN_RADIUS})`,
    );
  ...
  const summary = {
    pr: "https://github.com/lkamak/dimensional/pull/19",
