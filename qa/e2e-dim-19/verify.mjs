...
const PR_COMMIT =
  process.env.GIT_COMMIT || "87be6dd619a6a2aa46268feb30ae41869b33259f";
...
async function findRotateHandle(page) {
  await bindStage(page);
  return page.evaluate(
    ({ fill: expectedFill, stroke: expectedStroke }) => {
      const stage = window.__qaStage;
      if (!stage) return null;
      ...
      for (const c of circles) {
        ...
        if (fill === expectedFill && stroke === expectedStroke) {
...
async function countRotateHandles(page) {
  ...
  return page.evaluate(
    ({ fill: expectedFill, stroke: expectedStroke }) => {
      ...
      return stage
        .find("Circle")
        .filter((c) => c.fill() === expectedFill && c.stroke() === expectedStroke)
        .length;
    },
    { fill: HANDLE_FILL, stroke: HANDLE_STROKE },
  );
...
async function getFurnitureGroupRotation(page) {
  ...
  return page.evaluate(
    ({ fill: expectedFill, stroke: expectedStroke }) => {
      const stage = window.__qaStage;
      if (!stage) return null;
      // Furniture groups have a Rect child + Text; handle may be a sibling Group
      const groups = stage.find("Group");
      for (const g of groups) {
        const circles = g.find("Circle");
        const hasHandle = circles.some(
          (c) => c.fill() === expectedFill && c.stroke() === expectedStroke,
        );
...
async function main() {
  ...
  try {
    ...
    await pause(page, 700);
    await selectItem(page, "couch");
    const minZoom = await getStageMeta(page);
    const minHandle = await findRotateHandle(page);
    const minScreenR =
      minHandle && minZoom ? minHandle.radius * minZoom.scale : null;
    await shot(page, "15-zoom-min");

    await zoomToward(page, 3.5);
    await pause(page, 700);
    await selectItem(page, "couch");
    const maxZoom = await getStageMeta(page);
    const maxHandle = await findRotateHandle(page);
    const maxScreenR =
      maxHandle && maxZoom ? maxHandle.radius * maxZoom.scale : null;
    ...
    const sizeOk =
      ...
      minScreenR != null &&
      maxScreenR != null &&
      Math.abs(midScreenR - HANDLE_SCREEN_RADIUS) < 0.8 &&
      Math.abs(minScreenR - HANDLE_SCREEN_RADIUS) < 0.8 &&
      Math.abs(maxScreenR - HANDLE_SCREEN_RADIUS) < 0.8 &&
      Math.abs(midScreenR - minScreenR) < 0.8 &&
      Math.abs(midScreenR - maxScreenR) < 0.8;
    record(
      "AC8",
      "The handle remains a usable, visually consistent size at minimum and maximum supported zoom.",
      sizeOk,
      `screenR mid/min/max=${midScreenR?.toFixed?.(2)}/${minScreenR?.toFixed?.(2)}/${maxScreenR?.toFixed?.(2)} (target≈${HANDLE_SCREEN_RADIUS}; stage=${midZoom?.scale?.toFixed?.(2)}/${minZoom?.scale?.toFixed?.(2)}/${maxZoom?.scale?.toFixed?.(2)})`,
    );
  ...
  const summary = {
    pr: "https://github.com/lkamak/dimensional/pull/18",
