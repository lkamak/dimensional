...
const PR_COMMIT =
  process.env.GIT_COMMIT || "2543ae1ae33cfa0b90f93599c4295e4354373e71";
...
async function countRotateHandles(page) {
  ...
  return page.evaluate(() => {
    ...
    return stage
      .find("Circle")
      .filter((c) => c.fill() === "#ffffff" && c.stroke() === "#3d5a5b").length;
  });
}
  ...
  return page.evaluate(() => {
    ...
    for (const g of groups) {
      ...
      const hasHandle = circles.some(
        (c) => c.fill() === "#ffffff" && c.stroke() === "#3d5a5b",
      );
...
async function main() {
  ...
  const summary = {
    pr: "https://github.com/lkamak/dimensional/pull/10",
